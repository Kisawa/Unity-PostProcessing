Shader "PostProcessing/GTAO"
{
    ...
    SubShader
    {
		...
		inline float SampleViewNormal(float2 uv)
		{
        ...
        fixed4 frag_AO (v2f i) : SV_Target
        {
			...
			half jitter = GetRandom(i.uv) * stepRadius;
			float thetaAngle = UNITY_PI / (float)_SampleDirectionCount;
			float startAngle = GTAO_Noise(i.uv * _TexelSize.zw);
			UNITY_LOOP
			for (int ii = 0; ii < _SampleDirectionCount; ii++)
			{
				...
				for (int j = 0; j < _SampleStep; j++)
				{
					...
					float4 uvSlice = i.uv.xyxy + float4(uvOffset, -uvOffset);
